            ...
            if (response != 0) {
                ...
                for (var i = 0; i < cartItems.length; i++) {
                    itemHtml = "<div class='dropdown-item card-item-cont'>" +
                        ...
                        "               &nbsp;" +
                        "               <span class='badge badge-secondary ml-auto middle-h-item' id='card-item-quantity'>" + cartItems[i].quantity + " товар</span>" +
                        "          </div>" +
                        "      </div>";
                        ...
                        if (response == 1) {
                            $('.title-prod').each(function (e) {
                                ...
                                if (idProdTitle == idProdButton) {
                                    ...
                                    $('#modal-text').html(
                                        "Вы добавили <span class='badge badge-success modal-quantity-item'>" + quantity + "</span> " +
                                        "<span>" + $(this).html() + "</span> в <a href='page.php?name=cart'>корзину</a>."
                                    );
                            ...
                            $('.modalTitleItem').text("Ошибка");
                            $('#modal-text').html("Произошла ошибка, перезагрузите страницу или обратитесь в службу поддержки");
                            $('#modalItemAdded').modal('toggle');
                        } else {
                            $('.modalTitleItem').text("Вход");
                            $('#modal-text').html("Если у вас уже есть аккаунт, войдите <a class='btn btn-primary btn-sm' " +
                                "href='page.php?name=login'>Войти</a> <br> или " +
                                "<a class='btn btn-success btn-sm' href='page.php?name=registration'>Зарегестрируйтесь</a>");
                            $('#modalItemAdded').modal('toggle');
                            printNumItemCart(idUser);
                        }
                    }
